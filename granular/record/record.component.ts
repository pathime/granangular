import { Component, OnInit, Input, Output, EventEmitter } from 'angular2/core';
import { Control } from '../control/control.component';
import { ControlComponent } from '../control/control.component';
import { GranularService } from '../granular.service';
import { State } from '../granular.service';

export interface Recording {
    url: string;
    buffer: AudioBuffer;
}

@Component({
    selector: 'record',
    templateUrl: './record.component.html',
    styleUrls: ['../granular.component.css']
})

export class RecordComponent implements OnInit {

    @Input() inputNode: AudioNode;
    @Output() sendRecording = new EventEmitter<Recording>();
    @Output() microphoneDisabled = new EventEmitter();

    private ctx = new AudioContext();
    private recorder: any;
    private timeOut: any;

    public disabled: boolean = false;
    public state = State;

    constructor(private granularService: GranularService) { }

    ngOnInit() {
        this.initMicrophone();
    }

    private initMicrophone() {
        navigator['getUserMedia'] = navigator['getUserMedia'] ||
                                    navigator['webkitGetUserMedia'] ||
                                    navigator['mozGetUserMedia'] ||
                                    navigator['msGetUserMedia'];
        if (!navigator['getUserMedia']) this.setDisabled();
    }

    private setDisabled() {
        this.disabled = true;
        this.microphoneDisabled.emit({});
    }
    
    start() {
        if (!this.inputNode) {
            navigator['getUserMedia']({audio: true}, stream => {
                this.granularService.currentState = State.Recording;
                let microphone = this.ctx['createMediaStreamSource'](stream);
                this.recorder = new Recorder(microphone);
                this.recorder.record();
                this.timeOut = setTimeout(()=>this.stop(), 5000);
            }, error => this.setDisabled());
        } else {
            this.recorder = new Recorder(this.inputNode);
            this.recorder.record();
        }
    }

    stop() {
        this.granularService.currentState = State.Paused;
        clearTimeout(this.timeOut);
        let buffer, soundUrl;
        this.recorder.stop();
        this.recorder.getBuffer(buffers => {
            buffer = this.ctx.createBuffer( 2, buffers[0].length, this.ctx.sampleRate );
            buffer.getChannelData(0).set(buffers[0]);
            buffer.getChannelData(1).set(buffers[1]);
            if (soundUrl) prepareRecording();
        });
        this.recorder.exportWAV(blob => {
            soundUrl = URL.createObjectURL(blob);
            if (buffer) prepareRecording();
        });
        let prepareRecording = () => {
            this.sendRecording.emit({
                url: soundUrl,
                buffer: buffer
            });
        };        
    }

    ngOnDestroy(): void {

    }
}